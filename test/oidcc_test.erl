...
add_openid_provider_test() ->
    ...
    Id = <<"123345456">>,
    Config = #{client_id => <<"123">>, client_secret => <<"secret">>},
    {ok, RandomId, MyPid} = oidcc:add_openid_provider(ConfigEndpoint, LocalEndpoint, Config),
    {ok, Id, MyPid} =
        oidcc:add_openid_provider(ConfigEndpoint, LocalEndpoint, maps:put(id, Id, Config)),
...
create_redirect_url_test() ->
    ...
    Config2 = #{scopes => ["email", <<"profile">>, openid], state => State},
    Config3 =
        #{scopes => [email, profile, openid],
          state => State,
          nonce => Nonce},
    Config4 =
        #{scopes => ["email", <<"openid">>], url_extension => #{<<"other">> => <<"green">>}},
    ...
    ExpUrl1 =
        <<"https://my.provider/auth?scope=openid&response_type=code&client_id=123&redirect_uri=https%3A%2F%2Fmy.server%2Freturn&test=id">>,
...
retrieve_and_validate_token_test() ->
    ...
    ValidateFun =
        fun(TokenMap, Provider, _Nonce, _NoneAllowed) ->
           Provider = ProviderId,
           #{id := IdToken} = TokenMap,
           {ok, #{id => #{}}}
        end,
...
retrieve_user_info_test() ->
    ...
    HttpFun =
        fun(Method, Url, _Header, _UseCache) ->
           Method = get,
    ...
    BadToken =
        #{access => #{token => AccessToken},
          ...
          refresh => #{}},
    {ok, #{name := <<"joe">>}} = oidcc:retrieve_user_info(GoodToken, ProviderId),
    {ok, #{name := <<"joe">>}} = oidcc:retrieve_user_info(AccessToken, ProviderId, GoodSub),
    {ok, #{name := <<"joe">>}} = oidcc:retrieve_user_info(AccessToken, ProviderId),
    {error, bad_subject} = oidcc:retrieve_user_info(BadToken, ProviderId),
    {error, bad_subject} = oidcc:retrieve_user_info(AccessToken, ProviderId, BadSub),
    true = meck:validate(oidcc_openid_provider),
    true = meck:validate(oidcc_openid_provider_mgr),
    true = meck:validate(oidcc_http_util),
    meck:unload(oidcc_openid_provider),
