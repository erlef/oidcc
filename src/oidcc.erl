...
-spec add_openid_provider(binary(), binary(), map()) ->
                             {ok, Id :: binary(), Pid :: pid()} | {error, Reason :: any()}.
add_openid_provider(IssuerOrConfigEP, LocalEndpoint, AdditionalConfig) ->
    BasicConfig =
        #{name => <<"OpenId Connect Provider">>,
          ...
          static_extend_url => #{},
          registration_params => #{}},
    ForceUpdate = #{issuer_or_endpoint => IssuerOrConfigEP, local_endpoint => LocalEndpoint},
...
%% scope being openid
%% @end
-spec create_redirect_url(binary()) -> {ok, binary()} | {error, provider_not_ready}.
...
%% @end
-spec retrieve_user_info(map() | binary(), binary()) -> {ok, map()} | {error, any()}.
retrieve_user_info(#{access := _,
                     id := _,
                     refresh := _} =
                       TokenMap,
                   ProviderIdOrPid) ->
...
-spec retrieve_user_info(Token, ProviderIdOrPid, Subject) -> {ok, map()} | {error, any()}
    when Token :: binary() | map(),
         ProviderIdOrPid :: binary() | pid(),
         Subject :: binary() | undefined.
retrieve_user_info(Token, ProviderIdOrPid, Subject) ->
    {ok, #{userinfo_endpoint := Endpoint}} = get_openid_provider_info(ProviderIdOrPid),
...
retrieve_fresh_token(RefreshToken, Scopes, OpenIdProvider) ->
    ...
    BodyQs0 = [{<<"refresh_token">>, RefreshToken}, {<<"grant_type">>, <<"refresh_token">>}],
...
%% @end
-spec introspect_token(Token, ProviderOrConfig) -> {ok, map()} | {error, any()}
    when Token :: binary() | map(),
         ProviderOrConfig :: binary() | map().
introspect_token(TokenMapIn,
...
add_authentication_code_verifier(QsBodyList,
                                 Header,
                                 basic,
                                 ClientId,
                                 Secret,
...
return_json_info({ok, #{status := 200, body := Data}}) ->
    try jsone:decode(Data, [{keys, attempt_atom}, {object_format, map}]) of
        Map ->
            {ok, Map}
    catch
        Error ->
