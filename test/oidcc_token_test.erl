...
extract_test() ->
    ...
    ExpectedOutput =
        #{access =>
          ...
          id =>
              #{token =>
                    <<"eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE0NjA5NjI0NzUsImlzcyI6Imh0dHBzOi8vcHJvdG9uLnNjYy5raXQuZWR1Iiwic3ViIjoiam9lIiwiYXVkIjoiMTIzIiwiaWF0IjoxNDYwOTYyMTc0LCJhdXRoX3RpbWUiOjE0NjA5NjIxNzR9.bJUAXVktgAAIIlw6fshlF035NfpNJ4aF8VfoIt4Kf5UyfTBzG1m9AGClvEFWieWnSJ6AxAV5dLJd3L-_tPs3cM9qUxssuY5CoKtc659X0B8cAOR4vK3ImyEAnipnUGYXcP3Ju8vkqtP75_GOUnbWEtNtxT-GK_2ysRQyF6wpRQHL-lj5u-lhZBHXDDJmGB5A4pex_zdtemlPFc9Ij_XDbmMEh-BoT9r9orC7prJ-ih3cGz3YKfOyxYQO8VPueucwouFcR8FCKasN3IOgyPfUnJl5wtsbm54u94dgu_uMpaIOeDnWbyEUsFldUvhGVJKlXlsk3Q34sFR-0FT1IOvCke">>,
                claims => undefined},
          scope => #{scope => <<"openid profile">>, list => [<<"openid">>, <<"profile">>]}},
    ExpectedOutput = oidcc_token:extract_token_map(RawData, <<"openid profile">>),
    ok.
...
validate_fail_field_test() ->
    ...
    {error, {required_fields_missing, _}} =
...
introspect_test() ->
    Token1 =
        <<"{\"active\":true,\"scope\":\"openid profile\",\"client_id\":\"id1\", \"username\":\"joe\", \"exp\":234}">>,
    Exp1 =
        #{active => true,
          scope => #{scope => <<"openid profile">>, list => [<<"openid">>, <<"profile">>]},
          username => <<"joe">>,
...
to_key(#'RSAPrivateKey'{modulus = Modulus,
                        publicExponent = PubExp,
                        privateExponent = PrivExp}) ->
    E = base64url:encode(
            binary:encode_unsigned(PubExp)),
    N = base64url:encode(
            binary:encode_unsigned(Modulus)),
    D = base64url:encode(
            binary:encode_unsigned(PrivExp)),
    #{kty => <<"RSA">>,
      n => N,
      e => E,
      d => D}.
